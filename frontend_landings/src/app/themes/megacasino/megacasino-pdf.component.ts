 ...
 */
@Component({
  selector: 'app-megacasino-pdf',
  template: `
    <div class="mg-catalogo-visor">
      <div class="mg-catalogo-hoja">
        @if (estado() === 'cargando') {
          <p class="mg-catalogo-aviso">Cargando PDF...</p>
        }

        @if (estado() === 'error') {
          <p class="mg-catalogo-aviso error">Error al cargar el PDF</p>

          @if (motivo()) {
            <p class="mg-catalogo-motivo">{{ motivo() }}</p>
            <p class="mg-catalogo-motivo"><code>{{ pdf }}</code></p>
          }
        }

        @if (estado() === 'vacio') {
          <p class="mg-catalogo-aviso">Todavía no hay un catálogo publicado.</p>
        }

        <!-- Aquí se van añadiendo los canvas, uno por página. -->
        <div #paginas class="mg-catalogo-paginas"></div>
      </div>

      @if (pdf) {
        <button type="button" class="mg-catalogo-descargar" (click)="descargar()"
                title="Descargar PDF">
...
export class MegacasinoPdfComponent implements AfterViewInit, OnDestroy {
  private doc = inject(DOCUMENT);

  @ViewChild('paginas') private contenedor?: ElementRef<HTMLDivElement>;

  /**
   * Qué sección lleva el PDF.
   *
   * Este tema tiene dos páginas de PDF, el catálogo y la carta del
   * restaurante, y las dos usan este visor. Cuál toca se deduce de la
   * dirección, no se recibe de fuera: así `catalogo-page.component.ts`, que es
   * común a todos los temas, se queda sin tocar.
   */
  get seccionClave(): 'mega-catalogue' | 'mega-restaurant' {
    const ruta = this.doc.defaultView?.location.pathname ?? '';

    return ruta.replace(/\/+$/, '').endsWith('/restaurante')
      ? 'mega-restaurant'
      : 'mega-catalogue';
  }
  ...
  /**
   * Solo guarda. La ruta del PDF se arma en ngAfterViewInit.
   *
   * No se calcula aquí porque en ese momento la vista aún no existe, y de paso
   * queda junto al resto del arranque.
   */
  ...
  get nombre(): string {
    return this._data?.venue?.name ?? 'Mega Casino';
  }
  ...
  /** Arma la ruta del archivo con la sección que toque. */
  private resolverPdf(): void {
    const seccion = (this._data?.sections?.[this.seccionClave]?.[0] ?? {}) as { pdfWeb?: string };
  ...
  descargar(): void {
    ...
    a.href = this.pdf;
    a.download = this.seccionClave === 'mega-restaurant' ? 'carta.pdf' : 'catalogo.pdf';
    a.click();
  }
}