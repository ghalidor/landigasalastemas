        ...
        public async Task<int> Handle(CreateContentItemCommand request, CancellationToken cancellationToken) {
            ...
            try {
                ...
                if(request.SectionKey == "seo") {
                    ...
                    /*  El dominio propio se guarda sin barra final: el canonical
                        lo usa tal cual y una barra de mas cuenta como otra
                        direccion para los buscadores.                      */
                    var dominio = seo?["SiteUrl"]?.ToString()?.Trim().TrimEnd('/');
