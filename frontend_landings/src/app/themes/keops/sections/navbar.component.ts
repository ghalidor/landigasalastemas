 ...
 */
@Component({
  selector: 'app-keops-navbar',
  imports: [RouterLink, ScrollAnclaDirective],
  template: `
    <nav class="kp-navbar">
      <div class="kp-navbar-contenido">
        <a [routerLink]="inicio" class="kp-navbar-logo">
          <img [src]="logo" [alt]="nombre" />
        </a>

        <button type="button" class="kp-navbar-menu" [class.abierto]="menuAbierto"
                (click)="menuAbierto = !menuAbierto"
                aria-label="Menú">
          <i class="fas" [class.fa-bars]="!menuAbierto" [class.fa-xmark]="menuAbierto"></i>
        </button>

        <ul class="kp-navbar-enlaces" [class.abierto]="menuAbierto">
          @for (e of enlacesVisibles; track e.ancla) {
            <li>
              <a [href]="'#' + e.ancla" [appScrollAncla]="e.ancla"
                 (click)="menuAbierto = false" class="kp-item">{{ e.nombre }}</a>
            </li>
          }
...
export class KeopsNavbarComponent {
  @Input() logo = '';
  @Input() nombre = '';
  ...
  get enlacesVisibles(): Enlace[] {
    const lista: Enlace[] = [
      ...
      { nombre: 'Keops Club', ancla: 'club', visible: this.hayClub },
  ...
  /*  Los SVG del proyecto original de Keops. Se dibujan cuando la sede no ha
