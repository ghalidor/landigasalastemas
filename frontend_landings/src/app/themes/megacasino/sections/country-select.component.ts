...
/**
 * Selector de país con bandera y buscador, como el del sitio de Mega.
 ...
 */
@Component({
  selector: 'app-mega-country',
  imports: [FormsModule],
  template: `
    <div class="mg-pais" [class.abierto]="abierto()">

      <button type="button" class="mg-pais-boton" (click)="alternar()"
        ...
        @if (elegida) {
          ...
          <span class="mg-pais-texto">{{ etiqueta(elegida) }}</span>
        } @else {
          <span class="mg-pais-texto vacio">{{ placeholder }}</span>
        }
      ...
      @if (abierto()) {
        <div class="mg-pais-panel">
          @if (conBuscador) {
            <div class="mg-pais-buscador">
          ...
          <ul class="mg-pais-lista">
            @for (o of filtradas; track o.value) {
              ...
              <li class="mg-pais-vacio">Sin resultados</li>
            }
          </ul>
        </div>
      }
...
export class MegaCountryComponent {
