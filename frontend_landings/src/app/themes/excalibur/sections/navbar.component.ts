 ...
 */
@Component({
  selector: 'app-excalibur-navbar',
  imports: [RouterLink, ScrollAnclaDirective],
  template: `
    <nav class="ex-navbar" [class.solida]="desplazado()">
      <div class="ex-navbar-contenido">
        <a [routerLink]="inicio" class="ex-navbar-logo">
          <img [src]="logo" [alt]="nombre" />
        </a>

        <button type="button" class="ex-navbar-menu" [class.abierto]="menuAbierto"
                (click)="menuAbierto = !menuAbierto"
                aria-label="Menú">
          <i class="fas" [class.fa-bars]="!menuAbierto" [class.fa-xmark]="menuAbierto"></i>
        </button>

        <ul class="ex-navbar-enlaces" [class.abierto]="menuAbierto">
          @for (e of enlacesVisibles; track e.ancla) {
            <li>
              <a [href]="'#' + e.ancla" [appScrollAncla]="e.ancla"
                 (click)="menuAbierto = false" class="ex-item">{{ e.nombre }}</a>
            </li>
          }
              ...
              <a [href]="r.enlace" target="_blank" rel="noreferrer" [title]="r.titulo"
                ...
                @if (r.imagen) {
                  ...
                  <!--  currentColor: el trazo toma el color del enlace, asi
                        ...
                        color fijo en el marcado no habia forma de cambiarlo
...
export class ExcaliburNavbarComponent {
  private doc = inject(DOCUMENT);

  /**
   * Si la barra ya no esta sobre la portada.
   *
   * Empieza transparente porque la portada arranca pegada arriba del todo, y
   * al bajar se vuelve azul oscuro para que los enlaces se sigan leyendo.
   */
  readonly desplazado = signal(false);

  @HostListener('window:scroll')
  alDesplazar(): void {
    this.desplazado.set((this.doc.defaultView?.scrollY ?? 0) > 10);
  }
  ...
  get enlacesVisibles(): Enlace[] {
    const lista: Enlace[] = [
      ...
      { nombre: 'Excalibur Club', ancla: 'club', visible: this.hayClub },
  ...
  /*  Los SVG del proyecto original de Excalibur. Se dibujan cuando la sede no ha
