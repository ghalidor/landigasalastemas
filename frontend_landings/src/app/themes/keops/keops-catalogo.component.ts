 ...
 */
@Component({
  selector: 'app-keops-catalogo-page',
  template: `
    <div class="kp-catalogo-visor">
      <div class="kp-catalogo-hoja">
        @if (estado() === 'cargando') {
          <p class="kp-catalogo-aviso">Cargando PDF...</p>
        }

        @if (estado() === 'error') {
          <p class="kp-catalogo-aviso error">Error al cargar el PDF</p>

          @if (motivo()) {
            <p class="kp-catalogo-motivo">{{ motivo() }}</p>
            <p class="kp-catalogo-motivo"><code>{{ pdf }}</code></p>
          }
        }

        @if (estado() === 'vacio') {
          <p class="kp-catalogo-aviso">Todavía no hay un catálogo publicado.</p>
        }

        <!-- Aquí se van añadiendo los canvas, uno por página. -->
        <div #paginas class="kp-catalogo-paginas"></div>
      </div>

      @if (pdf) {
        <button type="button" class="kp-catalogo-descargar" (click)="descargar()"
                title="Descargar PDF">
...
export class KeopsCatalogoPageComponent implements AfterViewInit, OnDestroy {
