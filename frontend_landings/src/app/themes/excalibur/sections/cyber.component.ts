    ...
    <section class="ex-cyber" id="cyber" [style.background-image]="fondoCss">
      <div class="ex-contenido ex-cyber-fila">
        <div class="ex-cyber-texto">
          ...
          @if (data.pdfWeb) {
            @if (isPreview) {
              <span class="ex-boton-cyber inerte">
                {{ data.buttonText || 'Visita nuestro catálogo' }}
              </span>
            } @else {
              <a [routerLink]="['/', slug, 'cyber']" target="_blank" class="ex-boton-cyber">
                {{ data.buttonText || 'Visita nuestro catálogo' }}
              </a>
            }
          }
...
export class ExcaliburCyberComponent {
  @Input() data: ExcaliburCyber = {};
  @Input() carpeta = '';
