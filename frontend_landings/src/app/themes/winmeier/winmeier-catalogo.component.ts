 ...
 */
@Component({
  selector: 'app-winmeier-catalogo-page',
  template: `
    <div class="wm-catalogo-visor">
      <div class="wm-catalogo-hoja">
        @if (estado() === 'cargando') {
          <p class="wm-catalogo-aviso">Cargando PDF...</p>
        }

        @if (estado() === 'error') {
          <p class="wm-catalogo-aviso error">Error al cargar el PDF</p>

          @if (motivo()) {
            <p class="wm-catalogo-motivo">{{ motivo() }}</p>
            <p class="wm-catalogo-motivo"><code>{{ pdf }}</code></p>
          }
        }

        @if (estado() === 'vacio') {
          <p class="wm-catalogo-aviso">Todavía no hay un catálogo publicado.</p>
        }

        <!-- Aquí se van añadiendo los canvas, uno por página. -->
        <div #paginas class="wm-catalogo-paginas"></div>
      </div>

      @if (pdf) {
        <button type="button" class="wm-catalogo-descargar" (click)="descargar()"
                title="Descargar PDF">
...
export class WinMeierCatalogoPageComponent implements AfterViewInit, OnDestroy {
  ...
  @Input({ required: true }) set data(valor: VenueContent | null) {
    ...
    /*  Este tema tiene TRES paginas de PDF: el catalogo, el restaurante y el
        cyber. Cual toca se deduce de la direccion, no se recibe de fuera, para
        no tener que tocar catalogo-page.component.ts, que es comun a todos los
        temas.

        Faltaba el restaurante: al abrir /restaurante caia en el catalogo y
        buscaba el PDF en la seccion equivocada, asi que decia que estaba
        vacio aunque la carta si estuviera subida.                           */
    const ruta = (this.doc.defaultView?.location.pathname ?? '').replace(/\/+$/, '');

    const clave = ruta.endsWith('/cyber') ? 'wm-cyber'
      : ruta.endsWith('/restaurante') ? 'wm-restaurant'
      : 'wm-catalogue';
