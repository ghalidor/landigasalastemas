...
/**
 * Selector con bandera y buscador, como el del resto de temas.
 *
 * El desplegable nativo no admite imágenes ni filtrado, y con 241 países la
 * nacionalidad era inmanejable.
 *
 * Lo usan los cuatro campos del formulario: tipo de documento, sexo,
 * nacionalidad y el prefijo del celular. Los dos primeros van sin buscador y
 * sin bandera, porque sus opciones no traen `code`.
 */
@Component({
  selector: 'app-isla-country',
  imports: [FormsModule],
  template: `
    <div class="is-pais" [class.abierto]="abierto()">

      <button type="button" class="is-pais-boton" (click)="alternar()"
        ...
        @if (elegida) {
          ...
          <span class="is-pais-texto">{{ etiqueta(elegida) }}</span>
        } @else {
          <span class="is-pais-texto vacio">{{ placeholder }}</span>
        }
      ...
      @if (abierto()) {
        <div class="is-pais-panel">
          @if (conBuscador) {
            <div class="is-pais-buscador">
          ...
          <ul class="is-pais-lista">
            @for (o of filtradas; track $index) {
              <li>
              ...
              <li class="is-pais-vacio">Sin resultados</li>
            }
          </ul>
        </div>
      }
...
export class IslaCountryComponent {
