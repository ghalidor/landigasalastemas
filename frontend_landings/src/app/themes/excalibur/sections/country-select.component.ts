...
/**
 * Selector de país con bandera y buscador, como el del sitio de Excalibur.
 ...
 */
@Component({
  selector: 'app-excalibur-country',
  imports: [FormsModule],
  template: `
    <div class="ex-pais" [class.abierto]="abierto()">

      <button type="button" class="ex-pais-boton" (click)="alternar()"
              [attr.aria-expanded]="abierto()">
        @if (elegida) {
          <span class="fi fi-{{ elegida.code }}"></span>
          <span class="ex-pais-texto">{{ etiqueta(elegida) }}</span>
        } @else {
          <span class="ex-pais-texto vacio">{{ placeholder }}</span>
        }
      ...
      @if (abierto()) {
        <div class="ex-pais-panel">
          <div class="ex-pais-buscador">
          ...
          <ul class="ex-pais-lista">
            @for (o of filtradas; track o.value) {
              ...
              <li class="ex-pais-vacio">Sin resultados</li>
            }
          </ul>
        </div>
      }
...
export class ExcaliburCountryComponent {
