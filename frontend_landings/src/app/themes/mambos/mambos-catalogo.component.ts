 ...
 */
@Component({
  selector: 'app-mambos-catalogo-page',
  template: `
    <div class="mb-catalogo-visor">
      <div class="mb-catalogo-hoja">
        @if (estado() === 'cargando') {
          <p class="mb-catalogo-aviso">Cargando PDF...</p>
        }

        @if (estado() === 'error') {
          <p class="mb-catalogo-aviso error">Error al cargar el PDF</p>

          @if (motivo()) {
            <p class="mb-catalogo-motivo">{{ motivo() }}</p>
            <p class="mb-catalogo-motivo"><code>{{ pdf }}</code></p>
          }
        }

        @if (estado() === 'vacio') {
          <p class="mb-catalogo-aviso">Todavía no hay un catálogo publicado.</p>
        }

        <!-- Aquí se van añadiendo los canvas, uno por página. -->
        <div #paginas class="mb-catalogo-paginas"></div>
      </div>

      @if (pdf) {
        <button type="button" class="mb-catalogo-descargar" (click)="descargar()"
                title="Descargar PDF">
...
export class MambosCatalogoPageComponent implements AfterViewInit, OnDestroy {
