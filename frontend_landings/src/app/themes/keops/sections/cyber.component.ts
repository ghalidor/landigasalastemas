    ...
    <section class="kp-cyber" id="cyber" [style.background-image]="fondoCss">
      <div class="kp-contenido kp-cyber-fila">
        <div class="kp-cyber-texto">
          ...
          @if (data.pdfWeb) {
            @if (isPreview) {
              <span class="kp-boton-cyber inerte">
                {{ data.buttonText || 'Visita nuestro catálogo' }}
              </span>
            } @else {
              <a [routerLink]="['/', slug, 'cyber']" target="_blank" class="kp-boton-cyber">
                {{ data.buttonText || 'Visita nuestro catálogo' }}
              </a>
            }
          }
...
export class KeopsCyberComponent {
  @Input() data: KeopsCyber = {};
  @Input() carpeta = '';
